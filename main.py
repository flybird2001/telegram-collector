...
for channel_user, message in channel_messages_array:
    try:
        ...
        matches_username, matches_url, _ , _ , _ , _ , _ = find_matches(url_text_content)
        _ , _ , matches_shadowsocks, matches_trojan, matches_vmess, matches_vless, matches_reality = find_matches(config_text_content)
    ...
    for config in array_configuration:
        try:
            if config.startswith('ss'):
                ...
                non_title_config = f"SS-{id}@{ip}:{port}"
            ...
            if config.startswith('trojan'):
                ...
                non_title_config = f"TR-{id}@{ip}:{port}"
                country_config_dict[non_title_config] = config
            ...
            if config.startswith('vless'):
                ...
                non_title_config = f"VL-{id}@{ip}:{port}"
                country_config_dict[non_title_config] = config
            ...
            if config.startswith('vmess'):
                ...
                dict_params['ps'] = ''
                dict_params = {k: v for k, v in sorted(dict_params.items(), key=lambda item: item[0])}
                non_title_config = f"VM-{id}@{ip}:{port}"
